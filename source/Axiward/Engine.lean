...
/-- This conclusion reads the actual stored records. It does not establish
    that all external operations were registered or that a publication is true. -/
theorem complete_requires_recorded_results (s : State) (h : complete s = true) :
    (∃ root : Node, List.head? (Array.toList s.nodes) = some root ∧
      root.domain.published.isSome = true) ∧
    (∀ node : Node, List.Mem node (Array.toList s.nodes) →
      ∀ operation : Operation, List.Mem operation node.domain.workflow.operations →
        operation.result.isSome = true) := by
  obtain ⟨published, settled⟩ := complete_requires_proof_and_settlement s h
  refine ⟨?_, ?_⟩
  · refine ⟨s.nodes[0]'s.integrity.1, ?_, published⟩
    simpa only [List.head?_eq_getElem?, Array.getElem?_toList] using
      (Array.getElem?_eq_getElem s.integrity.1)
  · have counts : (s.nodes.map (fun node =>
        (node.domain.workflow.operations.filter (·.result.isNone)).length)).sum = 0 := by
      simpa only [Array.sum_eq_foldl, Array.foldl_map, pendingOperations] using settled
    intro node inNodes operation inOperations
    have countZero := (Array.sum_eq_zero_iff_forall_eq_nat.mp counts)
      ((node.domain.workflow.operations.filter (·.result.isNone)).length)
      (Array.mem_map_of_mem (Array.mem_toList_iff.mp inNodes))
    have noPending := (List.filter_eq_nil_iff.mp (List.length_eq_zero_iff.mp countZero))
      operation inOperations
    cases result : operation.result <;> simp_all

def newState (scope : Scope) (valid : validRequirements scope.requirements = true)
    (initialSource : Option Candidate := none) : State :=
  ⟨#[{ domain := initialDomain scope }], by
    simp [GraphIntegrity, domainCheck, edgesValid, publicationValid, initialDomain, valid],
    { schema := if initialSource.isSome then 4 else 3, initial := scope, initialSource }⟩

theorem newState_starts_open (scope : Scope) (valid : validRequirements scope.requirements = true)
    (initialSource : Option Candidate) :
    ∃ root : Node, List.head? (Array.toList (newState scope valid initialSource).nodes) = some root ∧
      root.domain.scope = scope ∧ root.domain.published = none ∧
      complete (newState scope valid initialSource) = false := by
  exact ⟨{ domain := initialDomain scope }, rfl, rfl, rfl, rfl⟩
...
def step (s : State) (request : Request) : Except Fault (Change s) := do
  ...
  | none =>
    if let .integrate serial check _ := request.command then
      unless check.current == sourceAt s.journal.entries s.journal.initialSource &&
          check.base == packageSource s.journal.entries request.node serial s.journal.initialSource do
        throw .staleRevision
      if let some reuse := check.reuse then
...
def restore (journal : Journal) : Except String State := do
  if journal.schema != 3 && journal.schema != 4 then throw "unsupported journal schema; expected schema 3 or 4"
  if journal.schema == 3 && journal.initialSource.isSome then
    throw "schema 3 cannot bind an initial source"
  if journal.schema == 4 && journal.initialSource.isNone then
    throw "schema 4 requires an initial source"
  if journal.initialSource.any (fun source => source.tree.isEmpty) then
    throw "missing initial source tree"
  ...
  if valid : validRequirements journal.initial.requirements = true then
    journal.entries.foldlM (init := newState journal.initial valid journal.initialSource) fun s entry => do
