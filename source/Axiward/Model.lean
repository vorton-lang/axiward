...
structure Journal where
  schema : Nat := 3
  initial : Scope
  initialSource : Option Candidate := none
  entries : List Entry := []
  deriving Repr, BEq, DecidableEq, ToJson, FromJson

def sourceAt (history : List Entry) (initialSource : Option Candidate := none) : Option Candidate :=
  history.foldl (fun source entry => match admittedPublication entry with
    | some p => some p.candidate
    | none => source) initialSource

/-- The journal prefix at acquisition fixes the merge base. An adopted source
    is an unverified starting point, never an admitted publication. -/
def packageSource (history : List Entry) (node serial : Nat)
    (initialSource : Option Candidate := none) : Option Candidate :=
  sourceAt (history.takeWhile (fun e => !(e.request.node == node && e.reply == .acquired serial))) initialSource
