...
def startExperiment (repo : FilePath) (id owner : String) (node serial : Nat) (directory : FilePath) : IO Json := do
  ...
  let allocationEntries := loaded.state.journal.entries.takeWhile (fun e =>
    !(e.request.node == node && e.reply == .acquired serial))
  let snapshot ← Git.decode (restore { loaded.state.journal with entries := allocationEntries })
  let some input := snapshot.nodes[node]? | throw (IO.userError "package input node missing")
  let candidate ← Verifier.importCandidateForPolicy repo input.domain.scope.policy directory
