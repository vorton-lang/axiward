...
TOOLS = {
    ...
    "experiment": ("Run a candidate under trials/<trial>/ through the frozen project checker. Records intent first; never replays lost work.",
...
class Adapter:
    ...
    def call(self, name, data, call_id):
        ...
        if name in ("submit", "prepare", "resume"):
            ...
            if name != "resume":
                information = self.cli("package-info", self.repo, self.worker, node, serial)
                assignment = information["action"]
                if assignment == "execute":
                    files = self.cli("candidate-files", self.repo, self.worker, node, serial, directory)
                else:
                    files = {"refine": ["plan.json", "Refinement.lean"],
        ...
        if name == "experiment":
            ...
            trial = self.local(directory, "trials", data["trial"])
            files = self.cli("candidate-files", self.repo, self.worker, node, serial, trial)
            sealed = self.stage(trial, files, missing_allowed=True)
