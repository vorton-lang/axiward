...
def mergeCase (repo : FilePath) (scope : Scope) (queue proofs kind : String) : IO Unit := do
  ...
  let seedScope ← Policy.restrictScope repo scope [0]
  let aScope ← Policy.restrictScope repo scope [1]
  let bScope ← Policy.restrictScope repo scope [2]
  let todoScope ← Policy.restrictScope repo scope [3, 4, 5]
  ...
  if kind == "merged" then
    ...
    require ((reason.splitOn (if kind == "merge-conflict" then "source conflict" else "candidate build")).length > 1)
...
def metadataCase (repo source : FilePath) (scope : Scope) (kind : String) : IO Unit := do
  if kind == "revision" then
    ...
    require ((impact.getObjValAs? (List String) "changedRequirements").toOption == some (scope.requirements.map (·.id)))
      "revision did not invalidate the changed acceptance contract"
...
def compositionCase (repo : FilePath) (scope : Scope) (queue proofs : String) : IO Unit := do
  ...
  let certificate ← Git.tree repo none #[⟨"fixture.txt", receipt⟩]
  let left ← Policy.restrictScope repo scope [0, 1, 2]
  let right ← Policy.restrictScope repo scope [3, 4, 5]
  let advance (s : State) (actor : Actor) (command : Axiward.Command) (node : Nat := 0) : IO State := do
  ...
  let _ ← Git.resolve repo s!"{head}:.axiward/compositions/10/kernel.json"
  ...
  try
    ...
    if kind == "refinement" || kind == "omitted-clause" then
      ...
      if kind == "refinement" then
        ...
        let _ ← Git.resolve repo s!"{output.certificate}:.lake/build/lib/lean/Refinement.olean"
        let _ ← Git.resolve repo s!"{output.certificate}:kernel-check.json"
      else
        require (result.verdict == .rejected "refinement build did not pass") "omitted goal did not fail real proof checking"
    ...
    if kind != "missing-proof" then
      IO.FS.writeFile (directory / "Proofs.lean")
        (if kind == "sorry" then proofs.replace "  intro α n q item room\n  change q.items.length < n at room\n  simp [subject, Queue.enqueue, room]" "  sorry" else proofs)
    let candidate ← Verifier.importCandidateForPolicy repo scope.policy directory
    ...
    if kind == "accepted" then
      ...
      let reason ← match reply with
        | .rejected _ reason => pure reason
        | .unresolved _ reason =>
          if kind == "missing-proof" then pure reason else throw (IO.userError s!"expected rejection: {repr reply}")
        | _ => throw (IO.userError s!"expected rejection: {repr reply}")
      let expected := if kind == "sorry" then "kernel" else if kind == "missing-proof" then "error" else "01-build"
      require (!reason.isEmpty) "missing rejection reason"
