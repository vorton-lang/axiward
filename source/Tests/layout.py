...
Delivery uses an existing disposable .work fixture in place; it never copies a repository.
"""
import argparse
import json
...
def main():
    ...
    if args.case == "delivery":
        ...
        assert repo.is_relative_to(source.parent / ".work"), "delivery requires a disposable .work fixture"
        target_name = "layout-delivery"
        before = git("rev-parse", "HEAD")
        delivered = cli("deliver", repo, target_name)
        destination = Path(delivered["directory"])
        assert destination == repo / target_name and destination.is_dir()
        manifest = (destination / "axiward-delivery.json").read_bytes()
        refused = cli("deliver", repo, target_name, success=False)
        ...
        named = cli("deliver", repo, target_name + "/next")
        ...
        assert {path.name for path in view.iterdir()} == {"AGENTS.md", ".codex", ".axiward"}
        ...
        if args.case == "execute":
            ...
            assert Path(material["path"]).read_bytes() == (view / ".axiward/policy/Axiward/Spec.lean").read_bytes()
