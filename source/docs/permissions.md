...
PowerShell 的路径提供器在父目录拒绝下可能把初始目录退到盘根。启动器通过 .NET `ProcessStartInfo.WorkingDirectory` 直接绑定 Lake 的 snapshot cwd；项目构建与核验使用同一实际目录，stdout/stderr 明确按 UTF-8 完整转发，退出码保持。没有为适配错误 cwd 修改 Audit 或其他规范文件。以上 ACL 和进程调用属于实现依赖与外部信任边界，正常核验通过不等于权限隔离性质的完整证明。
