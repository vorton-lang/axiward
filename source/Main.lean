...
def send (repo : String) (id : String) (actor : Actor) (command : Axiward.Command)
    (node : Nat) (loaded : Option Git.Loaded := none) : IO Unit := do
  emit (toJson (← Git.transact repo ⟨id, actor, command, node⟩ loaded))

def usage : String := "axiward init <new-absolute-project-directory> <user-confirmed-policy-dir> <lean-toolchain> (user-only)\n\
  axiward adopt <existing-repo> <reviewed-HEAD> <user-confirmed-policy-dir> <lean-toolchain> (user-only)\n\
  axiward validate-policy <policy-draft-dir> (read-only; validates format, not user agreement or semantics)\n\
  ...
  axiward revise-preview <repo> <request-id> <policy-draft-dir>\n\
  axiward revise <repo> <request-id> <user-confirmed-policy-dir> <review-token> (user-only)\n\
...
def main (args : List String) : IO UInt32 := do
  try
    ...
    | ["init", repo, policy, toolchain] =>
      ...
      let _ ← Policy.validateRoot policy
    ...
    | ["validate-policy", policy] =>
      emit (Json.mkObj [("acceptance", toJson (← Policy.validateRoot policy)),
        ("semanticConfirmation", toJson "required from user before init, adopt or revision")])
    | ["adopt", repo, expectedHead, policy, toolchain] =>
      Sandbox.requireCodex
      let _ ← Policy.validateRoot policy
      Git.checkRepository repo
      unless Git.objectId expectedHead && (← Git.resolve repo "refs/heads/main") == expectedHead do
        throw (IO.userError "repository changed; review the current HEAD before adoption")
      let scope ← Verifier.importPolicy repo policy toolchain
      let source ← Git.resolve repo s!"{expectedHead}:source"
      Verifier.validateSourceTree repo scope.policy source
      let head ← Git.adopt repo expectedHead scope
      emit (Json.mkObj [("adopted", toJson repo), ("head", toJson head),
        ("initialSource", toJson source), ("scope", toJson scope),
        ("rootClosed", toJson false), ("complete", toJson false)])
    | ["status", repo] =>
      ...
      emit (Json.mkObj [("head", toJson loaded.head), ("state", toJson loaded.state.domain),
        ("acceptance", toJson (← Policy.readManifest repo loaded.state.domain.scope.policy)),
    ...
    | ["begin", repo, id, owner, node, action] =>
      ...
      unless loaded.state.journal.entries.any (fun entry => entry.request.id == id) do
        Sandbox.requireCodex
      send repo id .controller (.begin owner (← actionOf action)) (← serialOf node) (some loaded)
    | ["submit", repo, id, owner, serial, directory, node] =>
      ...
      let some package := Interface.allocated loaded.state node serial
        | throw (IO.userError "unknown allocation")
      unless package.owner == owner do throw (IO.userError "wrong worker")
      ...
      let candidate ← match action with
        | .execute =>
          Verifier.importCandidateForPolicy repo package.input.policy directory
        | .refine => Refinement.importCandidate repo directory
        | .explore | .requestDecision => FlowIO.importCandidate repo directory action
      send repo id (.worker owner) (.submit serial candidate) node (some loaded)
    ...
    | ["candidate-files", repo, owner, node, serial, directory] =>
      let info ← Interface.packageInfo repo owner (← serialOf node) (← serialOf serial)
      let input : Scope ← Git.decode (info.getObjValAs? Scope "input")
      emit (toJson (← Verifier.candidatePaths (← Policy.readManifest repo input.policy) directory))
    ...
    | ["--version"] => IO.println "Axiward 0.2.0 (Lean 4.34.0; Windows; frozen project acceptance materials)"
