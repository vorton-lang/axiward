...
Each case clones ordinary Git data into a new output directory and checks one
independent read/access boundary. It never starts Codex or reruns the verifier.
"""
import argparse
import json
...
def main():
    ...
    run(["git", "clone", "--quiet", "--no-hardlinks", "--", original, repo])
    ...
    result = {"status": "passed", "case": args.case,
              ...
              "source": "retained real failure; original project read-only",
