...
**[先看当前项目状态](docs/roadmap.md)**：已有能力、验证边界和未完成验收。
...
共享源码三方合入、旧保证重验、路线失效回收及原始失败交接已有独立检查；已有 FIFO 示例已完成真实核验、默认交付和运行。当前源码尚未完成全量复验，真实 agent 持续推进和用户最终验收仍待完成。

- [使用说明](docs/product-guide.md)
...
每项检查含准备过程必须在 30 秒内完成；上列命令不是当前全量通过记录。具体未完成项见[项目状态](docs/roadmap.md#仍需完成)，真实核验与协议夹具的区别见[验证范围](docs/execution-slice.md#验证与边界)。权限隔离检查按用户要求跳过。
