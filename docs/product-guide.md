...
每个视图同时只给一个原生任务使用。每个新工作包都可以由全新 worker 执行：用 `session` 创建各自视图与身份，连接同一个 `$repo`。恢复仍然活动的原包时使用其所属视图；另一身份不能自动接管。工作包不会自动过期，正确恢复不依赖旧会话上下文。系统目前不自动启动或调度 worker。
...
`status` 返回项目地图、四个动作的评分与理由和项目层 `handoff`。`next` 返回固定动作、工作包编号、候选目录、规格和完整包级交接；重新取得视图、恢复核验或接收答复时也提供完整交接。评分是可替换的机械启发式，不代表最优性或证明；agent 可以选择另一项合规动作，领取后不能换类。
