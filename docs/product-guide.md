...
项目配置与 MCP 启动方式依据 [Codex MCP 文档](https://developers.openai.com/codex/mcp)。每个视图有独立的原生权限方案：拒绝项目根访问，显式允许自身工作空间读取及 `work/`、`tmp/` 写入；正式文件、`.git/`、`.axiward/` 和其他包空间均不向 worker 开放。控制端、适配器和核验目录也保持受保护。薄适配器提供获准材料和提交，候选核验另行受限。新嵌套布局的 Windows 原生隔离尚未执行验证。
