...
项目配置与 MCP 启动方式依据 [Codex MCP 文档](https://developers.openai.com/codex/mcp)。当前 R0 使用已批准的**完全访问权限 + 访问规则文本**；规则写在视图的 `AGENTS.md`。这不是操作系统级隔离。
...
用户确认的根规格、操作规范，以及 Lean 发行版、编译器、Git、原生 harness、适配器和操作系统仍属于信任前提。规格表达了什么与用户真正想要什么之间的差距，不由控制系统证明。当前也不防御完全访问权限下蓄意绕过规则的 worker 或恶意 Lean 元程序。
