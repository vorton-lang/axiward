...
普通 worker 通过薄适配器操作正式项目，原生命令只能修改自身 `.view/<包空间>/` 中的 `work/` 和 `tmp/`。项目根被拒读，自身空间有读取例外；正式文件、Git 数据、管理状态和其他包空间均受保护。控制端、适配器和核验目录另有拒读规则；候选构建与证明核验也在受限进程中执行。新嵌套布局尚未运行 Windows 原生隔离验证；此前验证状态见[项目状态](status.md)。
