...
用户通道与 worker 工具分离。包根直接放当前动作的候选文件，生成说明集中在 `AGENTS.md`、`ACTION.md`；封存使用文件白名单，元数据不混入候选。`.view/<唯一目录>/` 的启动配置绑定 worker 身份；空空间首次领取后绑定一个包，新包使用另一个空间与 session。原包可在原空间恢复，绑定从 Git 分配历史读取，不靠可写文件标记；活动包写操作仍要求原 owner。MCP 用户答复进入原调用并存于 Git。包输入保持领取快照，`current/` 记录与固定的 `node/` 材料明确区分。
