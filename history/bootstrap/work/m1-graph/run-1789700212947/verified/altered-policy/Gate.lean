...
theorem accepted (α : Type u) (n : Nat) :
    False ∧ Axiward.Q0.Created n (Axiward.Queue.empty α n).capacity (Axiward.Queue.empty α n).items ∧
    (∀ (q : Axiward.Queue α n) (x : α),
      Axiward.Q0.EnqueueRoom n q.items x (q.enqueue x).1 (q.enqueue x).2.items) ∧
    (∀ (q : Axiward.Queue α n) (x : α),
      Axiward.Q0.EnqueueFull n q.items (q.enqueue x).1 (q.enqueue x).2.items) ∧
    (∀ q : Axiward.Queue α n,
