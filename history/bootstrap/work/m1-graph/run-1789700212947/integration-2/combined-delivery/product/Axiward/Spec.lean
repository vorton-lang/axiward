...
-- 3. When full, explicitly fail and preserve the entire sequence.
def EnqueueFull (capacity : Nat) (before : List α)
    (accepted : Bool) (after : List α) : Prop :=
  capacity ≤ before.length → accepted = false ∧ after = before
