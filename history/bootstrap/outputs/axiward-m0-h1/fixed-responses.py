...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        wanted = cases[number] if number < len(cases) else {}
        executor = next((tool for tool in available if tool["name"] == wanted.get("tool", "exec_command")
                         and tool["namespace"] == wanted.get("namespace")), None)
        if number < len(cases) and executor:
            case = cases[number]
            if case.get("tool"):
                call_arguments = case.get("arguments", {})
            else:
                call_arguments = {"cmd": case["command"], "workdir": case["cwd"],
                                  "max_output_tokens": 1600, "yield_time_ms": 1000}
                call_arguments.update(case.get("toolArguments", {}))
                if "shell" in case:
                    call_arguments.update(shell=case["shell"], login=False)
            item = {"type": "function_call", "id": f"fc_h1_{number}",
                    "call_id": f"call_h1_{number}", "name": executor["name"],
