...
Shell approval requests are declined. In MCP mode, the test client authorizes
only the two predefined fixture tools. This is an ephemeral, local,
...
try:
    ...
    if args.mcp_probe:
        adapter = package / 'mcp-probe.py'
        cli = work / 'lab/tools/axiward_probe.exe'
        shutil.copyfile(adapter, run / 'mcp-probe-used.py')
        definition = ('mcp_servers.axiward_h1 = { command = ' + json.dumps(sys.executable)
                      + ', args = ' + json.dumps([str(adapter), '--exe', str(cli)])
                      + ', enabled = true }')
        overrides.extend(['-c', definition, '-c', 'mcp_servers.node_repl.enabled=false',
    ...
    def receive():
        ...
        if 'id' in item and 'method' in item:
            ...
            elif (args.mcp_probe and item['method'] == 'mcpServer/elicitation/request'
                  and item['params'].get('serverName') == 'axiward_h1'
                  and item['params'].get('message') in (
                      'Allow the axiward_h1 MCP server to run tool "status"?',
                      'Allow the axiward_h1 MCP server to run tool "submit"?')):
                # The approved H1 fixture allows these tools only on synthetic data.
