...
theorem enqueue_full (q : Queue α n) (item : α) :
    Q0.EnqueueFull n q.items item (q.enqueue item).1 (q.enqueue item).2.items := by
  intro full
  by_cases zero : n = 0
  · simp [Queue.enqueue, Nat.not_lt.mpr full, zero]
  · have positive : 0 < n := Nat.pos_of_ne_zero zero
    simp [Queue.enqueue, Nat.not_lt.mpr full, zero, positive]
...
def SatisfiesQ0 (α : Type u) (n : Nat) : Prop :=
  ...
  (∀ (q : Queue α n) (item : α),
    Q0.EnqueueFull n q.items item (q.enqueue item).1 (q.enqueue item).2.items) ∧
